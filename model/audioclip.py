...
ClipLogits = Tuple[
    ...
    Optional[torch.Tensor],  # audio x eeg
    Optional[torch.Tensor]   # image x eeg
]
...
class EAVCLIP(CLIP):
    ...
    def __init__(self,
        ...
        self.logit_scale_ae = torch.nn.Parameter(torch.log(torch.ones([]) * 100))
    ...
    def encode_eeg(self, eeg: torch.Tensor) -> torch.Tensor:
        return self.eeg(eeg.to(self.device))
    ...
    def encode_text(self,
        ...
        return super(EAVCLIP, self).encode_text(text_tokens)
    '''
    ...
    def forward(self,
        ...
        features: ClipFeatures = (audio_features, image_features, eeg_features)

        logit_scale_ai = torch.clamp(self.logit_scale_ai.exp(), min=1.0, max=100.0)
        logit_scale_ae = torch.clamp(self.logit_scale_ae.exp(), min=1.0, max=100.0)
        logit_scale_ie = torch.clamp(self.logit_scale.exp(), min=1.0, max=100.0)

        logits_audio_image = None
        logits_audio_eeg = None
        logits_image_eeg = None
        ...
        if (audio_features is not None) and (eeg_features is not None):
            logits_audio_eeg = logit_scale_ae * audio_features @ eeg_features.T

        if (image_features is not None) and (eeg_features is not None):
            logits_image_eeg = logit_scale_ie * image_features @ eeg_features.T

        logits: ClipLogits = (logits_audio_image, logits_audio_eeg, logits_image_eeg)
    ...
    def loss_fn(self, logits: ClipLogits, sample_weights: Optional[torch.Tensor] = None) -> Optional[torch.Tensor]:
        ...
        elif logits_audio_eeg is not None:
            batch_size = logits_audio_eeg.shape[0]
        elif logits_image_eeg is not None:
            batch_size = logits_image_eeg.shape[0]
        else:
            return None
        ...
        if logits_audio_eeg is not None:
            loss_ae = F.cross_entropy(
                logits_audio_eeg, reference, weight=sample_weights
            ) + F.cross_entropy(
                logits_audio_eeg.transpose(-1, -2), reference, weight=sample_weights
            )
            loss = loss + loss_at
            num_modalities += 1

        if logits_image_eeg is not None:
            loss_ie = F.cross_entropy(
                logits_image_eeg, reference, weight=sample_weights
            ) + F.cross_entropy(
                logits_image_eeg.transpose(-1, -2), reference, weight=sample_weights
            )
            loss = loss + loss_it
            num_modalities += 1
